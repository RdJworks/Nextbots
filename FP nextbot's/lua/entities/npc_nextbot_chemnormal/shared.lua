...
function ENT:DamagePoison()
if ( self.ThinkTimer == nil ) then self.ThinkTimer = CurTime() + 1 end
if ( self.ThinkTimer < CurTime() ) then
local TeamShit =  player.GetAll( )
        for k, v in pairs( TeamShit ) do
                if ( v:IsPlayer() and v:Alive() and v:GetPos():Distance( self:GetPos() ) < 100  ) then
                        v:TakeDamage( 2, self )
                        v:EmitSound( "vo/npc/male01/pain0"..math.random( 1, 9 )..".wav", 511, 100 )
...
function ENT:HaveEnemy()
	...
	if ( self:GetEnemy() and IsValid( self:GetEnemy() ) ) then
		...
		elseif ( self:GetEnemy() != nil  and self.Enemy:IsValid() ) then
			 if ( self:AttackPlayer() or self:DamagePoison() or self:IFoundSomeone() or self:JumpPosLadder() ) then
		else
		if
			( self:AttackProp()  or self:DamagePoison()  ) then
		end
		end	
		end
		return true
...
list.Set( "NPC", "npc_nextbot_chemnormal", {
	Name = "ChemNormalZombie",
	Class = "npc_nextbot_ChemNormal",
